...
def keys_check(df, cols_chaves):
    len(df[['companhia_formatted','datetime_partida_formatted', "id_voo",'datetime_chegada_formatted']].drop_duplicates()) == len(df)
    logger.info(f'check ; {datetime.datetime.now()}')
